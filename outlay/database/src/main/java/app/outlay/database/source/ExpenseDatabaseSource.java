...
import app.outlay.database.adapter.ExpenseDatabaseMapper;
...
public class ExpenseDatabaseSource implements ExpenseDataSource {
    ...
    public Observable<List<Expense>> getAll() {
        return Observable.create(subscriber -> {
            try {
                List<app.outlay.database.dao.Expense> dbExpenses = expenseDao.loadAll();
    ...
    public Observable<List<Expense>> saveAll(List<Expense> expenses) {
        return Observable.create(subscriber -> {
            try {
                List<app.outlay.database.dao.Expense> dbExpenses = mapper.fromExpenses(expenses);
    ...
    public Observable<Expense> saveExpense(Expense expense) {
        return Observable.create(subscriber -> {
            try {
                app.outlay.database.dao.Expense dbExpense = mapper.fromExpense(expense);
    ...
    public Observable<List<Expense>> getExpenses(Date startDate, Date endDate, String categoryId) {
        return Observable.create(subscriber -> {
            try {
                List<app.outlay.database.dao.Expense> dbExpenses = loadExpenses(
                        startDate,
                        endDate,
                        categoryId
                );
    ...
    public Observable<Expense> findExpense(String expenseId, Date date) {
        return Observable.create(subscriber -> {
            try {
                app.outlay.database.dao.Expense e = expenseDao.load(Long.valueOf(expenseId));
    ...
    public Observable<Expense> remove(Expense expense) {
        return Observable.create(subscriber -> {
            try {
                app.outlay.database.dao.Expense daoExpense = mapper.fromExpense(expense);
    ...
    private List<app.outlay.database.dao.Expense> loadExpenses(
            Date startDate,
            Date endDate,
            String categoryId
    ) {
        ...
        List<app.outlay.database.dao.Expense> expenses = expenseDao.queryBuilder().where(
                whereCondition,
